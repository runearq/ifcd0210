...
Consultas para ver todos los datos y campos de una tabla; en este caso "actor" */

SELECT * FROM actor;


/* Consulta para ver todos los datos y campos especificos de una tabla; tambien en la tabla "actor" */

SELECT first_name,last_name, last_update FROM actor;


/*  Con esta sentencia se genero para una consulta que muestra los clientes que estan están en la tabla rental */
...
/* Con esta sentencia se genero para consultar los apellidos de los actores y la cantidad de actores que tienen ese 
...
/* Con esta sentencia junto al comando LEFT OUTHER JOINT, se utilizo para la consulta de nombres y apellidos, 
...
/*  Con esta sentencia sentencia junto al comando INNER JOINT se genero para una consulta 
...
/* Se utilizó un par de consultas “Joint”, para  ello se tomó como ejemplo de la vida real dentro de Sakila
 y por ende de una tienda de DVD, la cuales pueden  producir una lista diaria de alquileres vencidos para que
  los clientes puedan ser contactados y pedirles que devuelvan sus DVD vencidos. Para una consulta de este tipo, 
 se buscan  películas en la tabla de alquiler con una fecha de retorno NULA y donde la fecha de alquiler sea más 
 antigua que la duración del alquiler especificada en la tabla de películas. Si es así, la película está vencida 
 y debemos producir el nombre de la película junto con el nombre del cliente y el número de teléfono. */
...
Con ésta sentencia se obtuvo una vista con los datos de films cuyos nombres empiezan por un actor o actriz;
para este caso Penélope.  */

CREATE VIEW penelope AS
SELECT first_name as nombre, last_name as apellido, actor_id as id
FROM actor
WHERE first_name = 'PENELOPE';

/* 
Creando una vista para ver los cinco primeros géneros que ocupàn los mayores ingresos en la tienda. */
...
Disparadores (trigger) para insertar una oferta de 7.50$ cuando el DVD tenga un precio de 10$ */
